...
def get_event_list(request):
    ...
    if eid != '':
       ...
       try:
        ...
        returnJsonResponse({'status':10022,'message':'query result is empty'}) 
       else:
    ...
    if name != '':
        ...
        if results:
            ...
            returnJsonResponse({'status':10022,'message':'queryresultisempty'})
...
def add_guest(request):
    ...
    result = Guest.objects.filter(realname=realname,phone = phone)   
    if result:
        return JsonResponse({'status':10023,'message':'user has exists'})   
    result = Event.objects.get(id=eid).status
    if not result:
       return JsonResponse({'status':10024, 'message':'event status is not available'})
...
def get_guest_list(request):
    ...
    if results: 
        for r in results:
            ...
            datas.append(guest)
        return JsonResponse({'status':200, 'message':'success', 'data':datas})
    else: returnJsonResponse({'status':10022,'message':'queryresultisempty'})
    if eid != '' and phone != '':
        guest = {} 
        try: 
